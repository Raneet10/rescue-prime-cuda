...
__global__ int64_t rescuePrime(int64_t *input_sequence, int64_t N, int64_t m, int64_t *mds, int64_t *round_constants, int64_t alpha, int64_t alphainv, int64_t rate) {
    finite_field *state = new finite_field[m];
...
__device__ int64_t* matrix_mult(int64_t *a, int64_t *b, int64_t N) {
    __shared__ int64_t aTile[TILE_WIDTH * TILE_WIDTH],
                     bTile[TILE_WIDTH * TILE_WIDTH];
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    int64_t sum = 0;
    aTile[threadIdx.y * TILE_WIDTH + threadIdx.x] = a[row * TILE_WIDTH + threadIdx.x];
    bTile[threadIdx.y * TILE_WIDTH + threadIdx.x] = b[threadIdx.y * N + col];
    __syncthreads();
    for (int i = 0; i < TILE_WIDTH; i++) {
        sum += aTile[threadIdx.y * TILE_WIDTH + i]* bTile[i * TILE_WIDTH + threadIdx.x];
    }
    c[row*N+col] = sum;
    return c;
}
