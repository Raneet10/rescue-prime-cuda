...
namespace rescue_prime {
    // rescue prime constants from https://aszepieniec.github.io/stark-anatomy/rescue-prime

    // https://quuxplusone.github.io/blog/2022/07/08/inline-constexpr/
    inline constexpr size_t prime_field_mod = 407 * (1 << 119) + 1;
    inline constexpr uint64_t m = 2;
    inline constexpr uint64_t capacity = 1;
    inline constexpr uint64_t rate = 1;
    inline constexpr uint64_t N = 27;
    inline constexpr uint64_t alpha = 3;
    inline constexpr uint64_t alphainv = 180331931428153586757283157844700080811

    inline constexpr size_t mds[4] = {270497897142230380135924736767050121214, 4, 
                                      270497897142230380135924736767050121205, 13};
    // inline constexpr inv_mds[4] = [210387253332845851216830350818816760948, 60110643809384528919094385948233360270, 
    //                     90165965714076793378641578922350040407, 180331931428153586757283157844700080811];                    
    
    inline constexpr size_t [108] = {174420698556543096520990950387834928928,
    ...
    template<size_t Fp>
    __global__ Fp rescuePrime(int64_t *input_sequence, int64_t N, int64_t m, int64_t *mds, int64_t *round_constants, int64_t alpha, int64_t alphainv, int64_t rate) ;

    template<size_t Fp>
    __device__ Fp* rescuePrimePermutation(int64_t N, int64_t m, int64_t *mds, int64_t *round_constants, int64_t alpha, int64_t alphainv, Fp *state) ;
}
